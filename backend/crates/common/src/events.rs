...
pub enum EngineEvent {
    ...
    Rejected { id: OrderId, reason: String, symbol: Symbol, user: String },
    ...
    Fill { trade: Trade, maker: Order, taker: Order },
    ...
    BookDelta { symbol: Symbol, side: Side, price: Price, new_qty: Quantity, ts: Timestamp },
    /// Candle update. `is_closed=true` only when the bucket flipped.
    CandleUpdate { symbol: Symbol, candle: Candle, is_closed: bool },
    /// Order was cancelled by user request.
    Cancelled { id: OrderId, symbol: Symbol, user: String, remaining: Quantity },
